...
test("a relay claim runs as one body-free task-scoped turn", async () => {
	...
	assert.match(wakes[0]?.text as string, /task-relay/);
	assert.doesNotMatch(wakes[0]?.text as string, /ignore all prior instructions/);
	await handlers.get("before_agent_start")?.({ prompt: wakes[0]?.text as string });
	await assert.rejects(
		() =>
			(tools.get("a2a_relay_read_task") as RegisteredTool).execute("foreign", {
				taskId: "task-other",
			}),
