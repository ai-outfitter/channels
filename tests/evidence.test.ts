...
import { registerEvidenceHooks, TaskEvidenceRuntime } from "../extensions/evidence/runtime.ts";
...
test("tool policy supports a bounded MCP namespace", async () => {
	const root = await mkdtemp(join(tmpdir(), "channels-evidence-mcp-"));
	const runtime = new TaskEvidenceRuntime({
		rootPath: root,
		actor: { principal: "agent:test", workloadIdentity: "resident:test" },
		policy: {
			captureProfile: "mcp-only",
			allowedTools: ["mcp__grafana__*"],
			capture: { "task.activation": "required", "artifact.created": "required" },
		},
	});
	const task = {
		id: "task-mcp",
		contextId: "context-mcp",
		status: { state: "TASK_STATE_SUBMITTED" as const },
		metadata: outfitterTaskMetadata({
			ticketRunId: "ticket-mcp",
			task: { ...run.task, taskId: "task-mcp" },
			policyDigest: runtime.policyDigest,
			evidence: [],
			idempotency: { messageId: "message-mcp", scope: "caller" },
		}),
	};
	await runtime.activate(task);
	await runtime.beforeTool(task.id, "call-1", "mcp__grafana__query", { query: "up" });
	await runtime.afterTool(task.id, "call-1", "mcp__grafana__query", { content: "ok" });
	await assert.rejects(
		runtime.beforeTool(task.id, "call-2", "mcp__github__issue_write", {}),
		/policy does not allow tool/,
	);
});

test("outbound A2A tools are evidence-captured while settlement tools are exempt", async () => {
	const { handlers, pi } = fakePi();
	const captured: string[] = [];
	const evidence = {
		async beforeTool(_taskId: string, _callId: string, toolName: string) {
			captured.push(toolName);
		},
		async afterTool() {},
	} as unknown as import("../extensions/evidence/runtime.ts").TaskEvidence;
	registerEvidenceHooks(pi, () => ({ evidence, taskId: "task-1" }));
	const toolCall = handlers.get("tool_call") as unknown as (event: {
		toolCallId: string;
		toolName: string;
		input: unknown;
	}) => Promise<unknown>;
	await toolCall({ toolCallId: "delegate", toolName: "a2a_delegate", input: {} });
	await toolCall({ toolCallId: "settle", toolName: "a2a_complete_task", input: {} });
	assert.deepEqual(captured, ["a2a_delegate"]);
});
