...
import { findBearerCredential, parseJson, readBoundedBody } from "../a2a/http.ts";
import {
...
import type { NativeDeliveryStateName, TaskLocator } from "../origins/types.ts";
import { isNativeDeliveryState } from "../origins/types.ts";
...
export async function a2aRelayConfigFromEnv(): Promise<A2aRelayServerConfig> {
	...
	const config: A2aRelayServerConfig = {
		...
		queuePath,
		originStorePath,
		a2a: await a2aConfigFromEnv(undefined, true),
...
function deliveryState(value: unknown): NativeDeliveryStateName {
	if (isNativeDeliveryState(value)) return value;
...
function authenticate(
	...
	const credential = findBearerCredential(header, credentials);
...
async function readJsonBody(request: IncomingMessage): Promise<Record<string, unknown>> {
	const text = await readBoundedBody(
		...
		() => new ConnectorError(413, "body_too_large", "request body exceeds the limit"),
	);
	const invalid = (): Error =>
		new ConnectorError(400, "invalid_json", "request body must be a JSON object");
	const value = parseJson(text, invalid);
