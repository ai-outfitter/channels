...
import { validateActivationInput } from "./types.ts";
...
function validateDeliveryInput(input: NativeDeliveryInput): void {
	if (!["pending", "delivered", "failed", "not-applicable"].includes(input.state)) {
		throw new Error("native delivery state is invalid");
	}
