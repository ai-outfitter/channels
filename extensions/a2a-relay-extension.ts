...
export default function a2aRelayExtension(
	...
	const queue = (claim: A2aRelayClaim): void => {
		...
		const turn: TaskTurn = {
			claim,
			wake: `[channels] relay A2A task ${taskId} awaits. Read it with a2a_relay_read_task, then settle it with a2a_relay_complete_task or a2a_relay_require_input.`,
			settled: false,
		};
		pending = turn;
		scheduleRenewal(turn);
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params) {
			...
			return {
				content: [{ type: "text" as const, text: renderTask(task.status.state, task.history) }],
				details: task,
			};
		},
	});
