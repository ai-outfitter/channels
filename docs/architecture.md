...
| JMAP mail | Direct `fetch` today; [`jmap-jam`](https://www.npmjs.com/package/jmap-jam) is a typed candidate for the adapter | One bounded [`Email/get`](https://www.rfc-editor.org/rfc/rfc8621.html) for the task locator's exact account and email ID | `Email/set` plus `EmailSubmission/set`, with `onSuccessUpdateEmail` moving the submitted draft to Sent | A deterministic delivery header supports exact non-draft `Email/query` lookup after a crash; a matching draft is reused on retry. Reply response IDs identify the outbound email. Creation IDs are request-scoped, so they are distinct per object and are not treated as cross-request idempotency keys. |
