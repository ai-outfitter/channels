...
| JMAP calendar | Work-producing | Validated CalendarAlert UID plus recurrence ID when present | Account-scoped alert occurrence signal | None; each occurrence creates one Task | EventSource has no replay cursor; the calendar event remains the durable schedule resource | None | None. Calendar Tasks complete or reject. | Process the exact scheduled occurrence named by the Task signal. | Provider-event dedupe on the validated occurrence key | None | Completion, rejection, no-op, or evidence only. | Parser, occurrence-dedupe, task-intake, and no-generic-wake tests. |
